...
export function railwayReservation(passengers, trains) {
  ...
  for (let i = 0; i < passengers.length; i++) {
    ...
    if (!train) {
      result.push({ name: p.name, trainNumber: p.trainNumber, class: null, status: "train_not_found" });
      continue;
    }

    if (train.seats[p.preferred] > 0) {
      train.seats[p.preferred]--;
      result.push({ name: p.name, trainNumber: p.trainNumber, class: p.preferred, status: "confirmed" });
    } else if (train.seats[p.fallback] > 0) {
      train.seats[p.fallback]--;
      result.push({ name: p.name, trainNumber: p.trainNumber, class: p.fallback, status: "confirmed" });
    } else {
      result.push({ name: p.name, trainNumber: p.trainNumber, class: p.preferred, status: "waitlisted" });
    }
  }
